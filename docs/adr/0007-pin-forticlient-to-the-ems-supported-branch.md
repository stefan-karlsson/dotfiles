...
A branch pin is a version ceiling, and apt neither downgrades nor removes a package on its own, so the repository records the branches it has moved off. A laptop enrolled on a superseded branch is recognized as this setup's own rather than as a foreign installation, is moved onto the pinned branch, and has the superseded branch's package purged before the pinned branch's version is installed, which is the sequence Fortinet documents for changing branch.
