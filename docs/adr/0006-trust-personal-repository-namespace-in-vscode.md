...
Folder trust is machine state held outside `settings.json`, so chezmoi writes the trust record atomically, only while VS Code is closed, and keeps a bounded set of backups.
