...
validate_requirements() {
  ...
  version_check() {
    ...
    allowed_418=("rhel almalinux rocky ol cloudlinux")
  ...
  if ! command -v timeout >/dev/null 2>&1; then
    die "REQUIREMENT FAILURE: timeout (coreutils) needs to be installed"
  fi
...
install_pkg() {
  ...
  # register the agent
  log_info "[+] Registering Huntress Agent"
  timeout 300 $huntress_agent register || die "Failed to register agent"
