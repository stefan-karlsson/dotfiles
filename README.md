...
```bash
bash -c "$(curl -fsSL "https://raw.githubusercontent.com/stefan-karlsson/dotfiles/main/scripts/<script-name>")" -- <arguments>
```

Just replace `<script-name>` and `<arguments>` with the desired values. Example:

```bash
bash -c "$(curl -fsSL "https://raw.githubusercontent.com/stefan-karlsson/dotfiles/main/scripts/create_alternative_chrome_shortcut.sh")" -- --force
```
